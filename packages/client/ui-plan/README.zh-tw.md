...
模型透過穩定的 `exit_plan_mode` 工具結束 plan mode；其 plan 評審走已組合的 Web question 通道。

## 模型體驗

間接地，透過 chip 派發的 `/plan off` 命令列：`@deepseek-ai/dsh-plan-mode` 擁有該命令列驅動的模型可見 policy 段、結束工具 schema 與已記錄狀態，本包只算繪投影並行送使用者同樣可以手敲的內容。
