...
The inference dashboard is a native DSH Settings section for live vLLM runtime status. It stays inside the existing Settings dialog rather than embedding or navigating to another application. Summary cards show running and waiting request totals, KV-cache occupancy when available, cumulative prompt and generated tokens, and scheduler preemptions. A searchable table also shows every vLLM numeric series in the bounded response, including its metric family, type, HELP description, labels, and exact Prometheus value.

The browser calls the authenticated `llm.metrics` RPC. The Host reads the deployment-owned Prometheus endpoint configured by `DSH_INFERENCE_METRICS_URL`, bounds each response, applies a deadline, and returns parsed vLLM families without the endpoint address or non-vLLM process metrics. The parser retains up to 10,000 vLLM series and rejects malformed or oversized results instead of presenting an incomplete table. A successful response supplies the next refresh interval. A failed scrape stops automatic polling, keeps a visible error state, and requires an explicit retry.
...
- **Metric labels are visible to authenticated users** — model names, engine ids, histogram buckets, and cache configuration labels exposed by vLLM appear in the complete table. The internal metrics URL and non-vLLM process/runtime series remain Host-only.
