...
推理仪表盘是 DSH 设置中的原生分节，用于显示实时 vLLM 运行状态。它留在现有设置对话框中，不嵌入或跳转到其他应用。卡片显示运行中与等待中的请求总数、可用时的 KV 缓存占用率、累计输入与输出 token，以及调度器抢占次数。

浏览器调用已认证的 `llm.metrics` RPC。Host 读取由部署通过 `DSH_INFERENCE_METRICS_URL` 配置的 Prometheus 端点，限制每次响应大小、设置截止时间、只解析画面所需的 vLLM 指标，并且不返回端点地址。成功响应会提供下一次刷新间隔。抓取失败会停止自动轮询、保留可见错误状态，并要求用户明确重试。
