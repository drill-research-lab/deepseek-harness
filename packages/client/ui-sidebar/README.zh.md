...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.workspaces`、`sidebar.pipelines` 与 `sidebar.settings` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。这里没有插件 store。
...
`sidebar.pipelines` 座位位于浏览器区域之下、页脚之上：功能插件（当前为 Pipelines）在这里注册其导航区块，使用同一份区域 owner share（wide 标志、展开请求）。页脚承载 `sidebar.settings`：侧边栏只渲染固定在底部的布局 slot，并共享其栏状态（`wide`）；ui-settings 在此注册触发行和设置面板。
