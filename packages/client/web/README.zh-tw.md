...
外殼擁有瀏覽器標題投影。選中帶有持久標題的工作階段時，它會算繪 `<session title> — <existing HTML title>` 並回應後續標題修訂；未選擇工作階段或選中無標題工作階段時，會保留現有標題；外殼解除安裝時復原標題。現有 HTML 標題仍是可設定的產品後綴。
...
- **有意採用一次性算繪**：UI 等待啟動 settle；只要一個設定項失敗，載入頁面就會保留並逐項顯示醒目的報告，不提供部分可用性（漸進式算繪將作為獨立項目復原）。
