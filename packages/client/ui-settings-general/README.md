...
The General section queries `auth.me` when it mounts and shows the verified username with a logout action above feature-owned settings rows. Loading, an empty response, and a failed request remain visibly distinct; a failed request exposes a retry action. The logout action clears the identity cookie and reloads into the unauthenticated redirect. The component keeps this state locally, so remounting or opening a browser under another authenticated session cannot reuse another user's identity.
