...
The General section queries `auth.me` when it mounts and shows the verified username and stable user id above feature-owned settings rows. The component keeps this result in mount-local state, so remounting or opening a browser under another authenticated session cannot reuse another user's identity.
