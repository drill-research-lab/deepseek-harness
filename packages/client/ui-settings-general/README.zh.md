...
「通用」分区挂载时查询 `auth.me`，并在各功能设置行上方显示已验证的用户名和一个登出操作。加载中、空响应与请求失败具有不同的可见状态，失败状态提供重试操作。登出操作会清除身份 cookie 并重新加载进入未认证重定向。组件只在本地状态中保存这些信息，因此重新挂载或以另一个已认证 session 打开浏览器时不会复用其他用户的身份。
