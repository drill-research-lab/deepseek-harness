...
`types.ts` 為 `HarnessSdkJsonRpcServer` 所服務協議的每個載荷命名：
...
- **無協議版本協商**——握手只攜帶 `serverInfo.version`（`0.0.1`，用戶端不校驗）；處於預發布階段，無相容承諾。
- **無取消與工作階段關閉方法**——用戶端放棄輪次的方式是關閉執行時期行程；見 [`dsh-sdk-jsonrpc-server` README](../server/README.md)。
- **server→client 請求是未使用的功能**——傳輸層支持，但伺服器從不傳送；Python SDK 的應答介面為未來審批流程預留。
