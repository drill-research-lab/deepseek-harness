...
| [`agent-loop/`](agent-loop/README.md) | 默認具體 agent 驅動器 | `ctx.agentLoop` |

`scope` 提供共享作用域原語。`agent` 負責公開約定，`agent-loop` 是其默認實作；擴充外掛程式相依性該 seam，從而保持驅動器可替換。`agent-default-model` 負責部署選擇，Agent 入口僅在工作階段自身沒有選擇時使用它。
