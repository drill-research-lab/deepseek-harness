...
指令读取使用可选 `ctx.fs` 提供方。该插件不会静态注入 `fs`，因此没有提供方的产品树仍可启动，指令加载在提供方出现前不执行任何操作。其受信任的 loader 探测与流式读取会携带一个以已解析配置 home 或项目根为根的 `read-only` 按调用策略，因此这些指令仍可通过 `fs-sandbox` 读取，同时不会授予候选 symlink 访问其根外部的权限，也不会放宽模型控制的文件系统读取。它会解析每个候选文件并对解析结果执行 stat，因此只有当路径最后一段的 symlink 目标在该策略下仍可读时才会跟随：指向根内常规文件的链接会加载目标内容，缺失路径或非文件目标（包括指向目录的链接）则已确认不存在。resolve 或 stat 异常会改为将该候选文件的 scope 标记为暂时不可用。前缀取消与动态工具取消会传播到解析、元数据探测与流式读取。文件加载后的提供方失败会视为暂时不可用，而非文件已删除的证据。
...
- **不含提供方的读取会跨越信任边界跟随指令 symlink**：Node 文件系统回退会解析并加载最终组件位于树外的 symlink 目标，将其作为较低优先级的工作区指引（它绝不会覆盖 system、developer 或用户直接下达的指令）。施加限制的 `ctx.fs` 会改为拒绝配置 home 或项目根外的目标；加载不受信任仓库时，请使用该提供方或 OS 沙箱。
