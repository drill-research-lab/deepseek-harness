...
由 agent 發起的搜尋會在寄出請求前一刻，向相應工作階段追加僅用於日誌的 `web/deepseek-search-llm-request` 工作階段事件。其中包含已解析端點、API 版本，以及傳送給 DeepSeek 且不含金鑰的精確 JSON 請求體；不包含標頭和憑據。寄出請求前發生憑據處理失敗或取消時不會建立事件；寄出請求後才發生 HTTP 或回應失敗時，本次請求嘗試仍保留持久記錄。在 agent 之外透過過程直接呼叫提供方時，沒有發起工作階段可供記錄。
