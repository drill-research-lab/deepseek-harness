...
- `ctx.sessionProjections.register(definition): () => void` 註冊一個領域的單元。key 重複或 `stateVersion` 非法都會 throw；註冊是掛在呼叫方 fiber 上的 effect，領域外掛程式解除安裝後其 key（連同快取的 cell）從後續驅動程式與快照中消失——用戶端將其讀作能力缺失。
...
- `ProjectionDefinition<K, S>`——`{ key, schema, init(), apply(state, event), view(state), stateVersion }`：由三個純同步函式外加若干聲明構成的狀態驅動程式計算單元（state-driven computation unit），絕不是一個不透明的 getter。

## 約定

- **框架負責驅動程式，領域負責計算。** 登錄檔只訂閱一次 `session/event`；每個已提交事件都會主動經過每個單元的 `apply`。領域不持有任何訂閱。cell（每工作階段每單元一份 `{state, observedSeq}`，以 WeakMap 為鍵）惰性建置——在事件串流過之後才註冊的單元，或讀取一個早於該註冊的工作階段，都在首次觸達時從 `init` 出發在記憶體日誌上摺疊。
- **同引用即無工作。** 對與單元無關的事件，`apply` 必須返回同一個狀態引用；驅動程式以 `Object.is` 把守變更流，因此不匹配的事件只花一次呼叫，不產生任何下游工作。
...
本包承擔能力 seam 的 Service Definition 與驅動程式角色：領域 host 外掛程式（如 `dsh-tool-todo`）貢獻單元，載體（`dsh-host-apiproxy`）消費快照與變更流，兩側互不相識。
...
- **主動驅動程式（eager drive）逐事件觸達每個單元**——按構造開銷很低（全量值規則、同引用閘門），但若出現熱點路徑，可加按單元的事件類型預過濾，約定不變。
