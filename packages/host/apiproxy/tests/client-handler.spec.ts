...
describe('config unary surface', () => {
  it('round-trips every settings/credentials/llm method with its own payload and value shape', async () => {
    ...
    const group = { id: 'deepseek-official', name: 'DeepSeek', keyConfigured: true, models: [{ id: 'deepseek-v4-flash', name: 'Flash' }] }
    const api = scriptedApi({
      settings: {
      ...
      llm: {
        ...
        metrics: record('llm.metrics', r => ok(r, {
    ...
    const metrics = await c.llm.metrics({})
