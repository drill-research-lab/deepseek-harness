...
透過 `usersRoot` 設定 deployment-owned persistence root。該欄位未提供或為空白時，提供方使用 `$DSH_HOME/users` 或 `~/.dsh/users`。Web bundle 會把 `DSH_USERS_HOME` 對映到 `usersRoot`；提供方本身不讀取這個環境變數。生產 Web 部署應設定 `DSH_USERS_HOME=/var/lib/dsh/users`。提供方以 `0700` mode 建立目錄，以 `0600` mode 建立私有檔案。這些 mode 限制其他 OS 使用者，但不會在同一個 DSH 行程內授權 request。

產品 profile bootstrap 擁有 Linux deployment writer lease，並在初始化 profile 或啟用此 provider 前取得該 lease。Web bundle 的 `DSH_USERS_HOME` 對映仍可獨立設定，但不能繞過以 resolved `DSH_HOME` 為鍵的 lease。可變 Web provider 把 `ctx.ownership` 作為另一項 service availability 要求，且 bootstrap 已在此前建立 process exclusion。
