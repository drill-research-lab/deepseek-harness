...
[目录选择 seam](../directory-picker/README.md) 的**应用内浏览后端**：`BrowseDirectoryPicker` 以 `browse` 能力注册 `ctx.directoryPicker`，在认证请求所有者的规范根目录下提供单层目录列举与子目录创建。它要求 `ctx.ownership`，并在每次调用时解析 `currentPrincipal()` 与 `resolveOwnerRoot()`，因此同一个 picker 实例会为不同所有者提供相互独立的目录树。宿主屏幕上不渲染任何东西，因此该后端能服务原生后端无法触及的远程客户端。

行为事实：`list` 不带路径时落在当前所有者的根目录，`crumbs` 从该根目录开始，不包含其外部祖先。显式目标与创建父目录会先规范化，再由 `isPathUnder()` 检查，因此绝对路径逃逸、`..` traversal、跨所有者路径以及解析到根目录外的符号链接都以 `directory-outside-owner-root` 失败。列举**只返回目录**并按名称排序；只跟随仍在根目录内的目录符号链接，断链、循环及逃逸链接都会跳过，并携带宿主判定的 `hidden` 标志（POSIX 点前缀约定）。`createDirectory` 不递归（父目录缺失是真实失败，不是要补造的层级），即便被直接调用也把名称校验为单个非空白段，并对规范化且已围堵的父目录执行操作。两个原语都拒绝非完全限定的显式路径——相对形态，以及 Windows 上 `isAbsolute` 会放行的无盘符有根形态（`\foo`、`/foo`）与不完整的 UNC 前缀（`\\`、`\\server`）——报 `directory-unreadable`／`directory-create-failed`，而不是任由 `resolve` 把它重定位到宿主进程 cwd 或当前盘符之下。单次 `list` 至多返回 `maxEntries` 行（配置项，默认 1000——GitHub 网页端对目录列举采用的同一上限），且层级以流式方式经过一个有界窗口，无论目录有多少子项内存都保持 O(maxEntries)：被截断的层级保留按名排序的头部、隐藏行计入上限、只探测窗口内候选，并报告 `truncated: true`，供客户端提示层级不完整（窗口内的断链符号链接不会从窗口外回填——发生过驱逐本身已把层级标记为截断）；窗口插入为二分查找、满窗尾部单次比较即拒绝，且 `list` 透传调用方的 `AbortSignal`，断连或超时会停止扫描而不是让它在调用方离开后继续。失败抛出 seam 的类型化 `DirectoryPickerError`。策略依据：[目录选择能力 seam Agent Note](../../../.agents/notes/implemented/architecture/2026-07-28-directory-picker-capability-seam.md)。
...
- **不枚举盘符根**——所有者根目录是最高可见层级；约定不会枚举其外部的 Windows 盘符。
