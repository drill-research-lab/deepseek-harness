...
当 `ctx.fs` 可用时，发现通过 `ctx.fs.listDir` 列出根，通过 `ctx.fs.readText` 读取 skill 文件，并通过文件系统服务探测 `.git`。这些受信任的 loader 调用会携带一个以已解析配置 home 或项目根为根的 `read-only` 按调用策略，因此当 `ctx.fs` 施加沙箱限制时，这些根仍然可用，同时不会授予 skill 文件 symlink 访问其根外部的权限，也不会放宽普通模型控制的读取。完整 skill 加载会将查找中止信号转发给文件系统元数据和内容读取。如果没有文件系统服务，提供方回退到可中止的 Node 文件系统 I/O，使最小本地上下文仍能加载 skill。已确认缺失的路径属于有效空状态；遇到格式错误或非文本条目时，提供方会发出警告并跳过；意外的发现或读取失败会使注册表快照不完整，系统不会因此用看似发生删除的结果替换上一份可用模型目录。
