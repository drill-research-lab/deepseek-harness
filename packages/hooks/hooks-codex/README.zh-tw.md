...
一個 Cordis 外掛程式，在 harness 的規範攔截點上執行使用者現有 **Codex** hook 設定的受支持子集。它是 hooks 子系統中採用 **Codex 方言** 的一側。方言無關原語來自 [`@deepseek-ai/dsh-hook-protocol`](../hook-protocol/README.md)；該橋接負責處理 Codex 形狀的 payload、matcher 模式和決策對映。

該橋接實作 Codex 當前 hook 協議的一個有意選取的子集：
...
設定只在載入時解析**一次**。`configPath` 是**行程級**設定：相對路徑在載入時根據行程啟動 cwd 解析，而非每工作階段解析（`TODO(per-session-hook-config)`）。讀取／解析失敗會被隔離處理（記錄 + 不註冊任何內容）；實際消費 matcher 的事件所帶的無效 matcher 正則屬於此類失敗，並報告其 pattern 與事件。只執行同步 `type: 'command'` hook；非 command 或 `async: true` hook 會被解析並跳過，同時記錄警告。hook 接受 `timeout` 或 `timeoutSec` alias；兩者都未設定時，使用協議參考預設值 `DEFAULT_HOOK_TIMEOUT_MS`（來自 `dsh-hook-protocol`，10 分鐘）。五個橋接支持點之外的事件會在解析時丟棄。
...
- **不支持的 hook 事件（Codex 當前 10 項中的 5 項）：** `PermissionRequest`、`PreCompact`、`PostCompact`、`SubagentStart` 和 `SubagentStop`。這些事件的設定會在解析期間靜默丟棄。比較基線是 Codex [官方 hook 參考](https://learn.chatgpt.com/docs/hooks)。
- **`SessionStart` 只支持部分功能：** 支持純 stdout 與 JSON `additionalContext`，但 hook 脫離執行，因此上下文可能錯過第一個請求（`TODO(session-start-gating)`）。
- **`UserPromptSubmit` 只支持部分功能：** 支持阻塞加純 stdout 或 JSON 上下文，但不會強制執行通用 `systemMessage` 和 `{"continue": false}` 控制。
- **`PreToolUse` 只支持部分功能：** 支持阻塞，但會忽略 `additionalContext`、`permissionDecision: "allow"` 和 `updatedInput`。每個工具都表示為 `tool_input: { command }`，因此非 shell 工具參數不會如實公開給 hook。
- **`PostToolUse` 只支持部分功能：** 支持阻塞回饋與 JSON `additionalContext`，但不會強制執行 `{"continue": false}`，非 shell 工具參數會縮減為 `{ command }`，結構化工具輸出會在 `tool_response` 中展平為文字。
- **`Stop` 只支持部分功能：** 阻塞會強制另一個模型輪次，但 `stop_hook_active` 始終為 `false`，`last_assistant_message` 始終為 `null`，且不會強制執行 `{"continue": false}`。因此，無條件阻塞 hook 會在每個步驟中強制 continuation，除非它自我限制（`TODO(stop-loop-guard)`）。
- **通用 payload 與輸出欄位只支持部分功能：** 每個已對映事件都報告靜態設定的 `model` 與 `permission_mode: "default"`，而非當前 Codex 執行時期值。`systemMessage` 會被記錄並觸發警告，但不呈現，`{"continue": false}` 會被記錄但不會應用 Codex 事件特定停止行為（`TODO(hook-continue-false)`）。
- **設定載入與執行只支持部分功能：** 一個行程級 `configPath` 會在載入時解析；尚未實作 Codex 的活動使用者層、項目層、工作階段層、系統／託管層和外掛程式層、信任控制與內聯 `config.toml` hook 形式（`TODO(per-session-hook-config)`）。只執行同步 `command` handler，忽略 `statusMessage` 與 `commandWindows` 等當前元資料，匹配 handler 序列執行，而非使用 Codex 的並行啟動語義。
