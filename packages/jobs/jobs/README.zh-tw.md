...
`outputLimitBytes` 是生產方擁有的模型呈現策略，會原樣攜帶到快照中。控制器在新增狀態或通知中繼資料後應用它；登錄檔不會重寫生產方輸出，也不會為省略此欄位的生產方虛構預設值。
...
透過生產方外掛程式和 [`dsh-tool-jobs`](../tool-jobs/README.md) 間接影響；它們會算繪 job id、輸出、狀態、取消和完成通知。
