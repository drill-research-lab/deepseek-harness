...
| `fs-sandbox/` | 強制沙盒的 `FileSystem`：把讀取限制在按呼叫的工作區根目錄，並按每次呼叫的模式約束寫入/編輯（只讀模式拒絕，工作區寫入模式限制在工作階段工作區與臨時根目錄內） | （註冊 `ctx.fs`） |
