...
二进制随包交付，覆盖所有受支持平台（macOS/Linux/Windows，x64/arm64），因此无需宿主 `rg` 安装，工具在每个部署上都注册。返回路径会相对于解析后的工作目录显示（调用方 agent（智能体）有会话 cwd 时使用该 cwd，否则使用 `process.cwd()`）。受限调用的显式搜索路径经规范化符号链接解析后，必须仍位于已解析的工作区根目录下。当解析出的策略设置了 `workspaceViewRoot` 时（本地 Linux 组合使用 `/workspace`），约束检查会先用 `fromWorkspaceView` 把该别名下的显式搜索路径映射到真实工作区根目录；ripgrep 的 argv 保留该别名，因为受限子进程的 mount namespace 恰好暴露该路径。远程或虚拟文件系统搜索仍需要特定提供方的搜索后端，因为本包运行共置进程。
