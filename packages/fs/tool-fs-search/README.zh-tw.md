...
二進位隨包交付，覆蓋所有受支援平臺（macOS/Linux/Windows，x64/arm64），因此無需宿主 `rg` 安裝，工具在每個部署上都註冊。返回路徑會相對於解析後的工作目錄顯示（呼叫方 agent（代理）有工作階段 cwd 時使用該 cwd，否則使用 `process.cwd()`）。受限呼叫的明確搜尋路徑經規範化符號連結解析後，必須仍位於已解析的工作區根目錄下。當解析出的策略設定了 `workspaceViewRoot` 時（本地 Linux 組合使用 `/workspace`），約束檢查會先用 `fromWorkspaceView` 把該別名下的明確搜尋路徑映射到真實工作區根目錄；ripgrep 的 argv 保留該別名，因為受限子行程的 mount namespace 恰好暴露該路徑。遠端或虛擬檔案系統搜尋仍需要特定提供方的搜尋後端，因為本包執行共置行程。
