...
| `fs-sandbox/` | 强制沙箱的 `FileSystem`：把读取限制在按调用的工作区根目录，并按每次调用的模式约束写入/编辑（只读模式拒绝，工作区写入模式限制在会话工作区与临时根目录内） | （注册 `ctx.fs`） |
