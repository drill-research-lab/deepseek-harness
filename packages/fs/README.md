...
| `fs-sandbox/` | Sandbox-enforcing `FileSystem`: confines reads to the per-call workspace root and fences write/edit by the per-call mode (read-only denies, workspace-write contains to the session workspace + temp roots) | (registers `ctx.fs`) |
