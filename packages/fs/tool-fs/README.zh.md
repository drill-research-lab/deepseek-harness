...
当解析出的策略带有 `workspaceViewRoot` 时（本地 Linux 组合设为 `/workspace`），每个工具会在 `ctx.fs.resolve` 之前用 `fromWorkspaceView` 把模型给出的路径映射到真实工作区根目录，并用 `toWorkspaceView` 把解析出的 `displayPath` 映射回去，用于它返回的每个路径——read/write/edit 信封、`path` 字段，以及 not-found / 非常规文件的消息。`ctx.fs` 中的施加限制不变，仍以真实根目录为准。
