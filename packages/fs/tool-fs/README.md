...
- **read** — resolves the session sandbox policy, passes it to one `ctx.fs.stat` (type + size routing + version) and then `readText`/`streamText`, builds the line window, then emits `fs/observed` with a plain `ctx.emit`. (1 stat.)
- **read_image** — validates the argument, extension, attachment availability, deployment media types, and the image-capable route before any I/O; then resolves the session sandbox policy and passes it to one `ctx.fs.stat` (recording an `absent` observation for a missing target, like `read`) and a bounded `ctx.fs.readBytes` capped at the smaller of `imageLimits.maxImageBytes` and `imageLimits.maxMessageImageBytes`; after that it calls `attachments.saveImage` and finally emits `fs/observed`. (1 stat.)
...
When the resolved policy carries `workspaceViewRoot` (the local Linux composition sets `/workspace`), each tool maps a model-supplied path onto the real workspace root with `fromWorkspaceView` before `ctx.fs.resolve`, and maps the resolved `displayPath` back with `toWorkspaceView` for every path it returns — the read/write/edit envelopes, the `path` field, and the not-found / not-a-file messages. Enforcement in `ctx.fs` is unchanged; it keys on the real root.
