...
The **`FileSystem`** (`ctx.fs`) defines the storage primitives in one execution world — resolve paths, expose canonical process paths and file URIs, test containment, read whole or streaming text, read bounded raw bytes, inspect/list metadata, write atomically, and apply a literal edit — without saying HOW. Every read and mutation accepts an optional per-call `SandboxExecutionPolicy`; bare backends ignore it, while a confining backend enforces it. Both mutations take their version guard **optionally**, so `ctx.fs` on its own remains a complete, unconstrained storage seam. This package also owns the `fs/*` policy event vocabulary the tool dispatches and the policy plugin listens for.
