...
按 agent（代理）分別記錄到日誌的 plan 協作狀態，提供由部署方設定的引導內容、用於直接進入的 `/plan [message]` 命令、用於直接結束的 `/plan off` 命令，以及經使用者評審的 `exit_plan_mode` 結束方式。Plan mode 是軟引導；沙盒模式和批准策略各自強制執行限制，且不讀寫 plan 狀態。
...
啟用時，`plan:policy` 會算繪已設定的 `section`。外掛程式始終註冊 `exit_plan_mode`，使工具 schema 在轉換期間保持穩定；其 execute 路徑只接受已啟用的 plan mode，且只有透過 `ctx.userQuestions` 獲得使用者明確批准後才結束。
...
組合 `ctx.commands` 時，該包會註冊 `/plan [message]`，並將參數恰好為 `off` 的情況保留給直接結束。不帶參數的 `/plan` 會啟用 plan mode；任何其他非空參數都會先啟用 plan mode，再透過 `agent.steer()` 提交，因此它會在 plan 引導下成為下一步驟的常規已記錄使用者訊息。`/plan off` 會選擇停用狀態，不傳送模型輸入；它還可以在啟用 plan mode 的待處理選擇由輪內 pre-step 追加之前將其取消。
...
該段在 plan mode 內穩定，但進入或結束會從順序 50 開始改變系統提示詞。
...
選填訊息的歷史 token 成本與單獨提交該文字相同；不帶參數的 `/plan` 和 `/plan off` 不增加 token。一次帶有切換通知的已啟用狀態結束會追加一條簡短且會保留的通知。

#### KV Cache 影響

使用者塊是僅附加的對話成長。進入或結束 plan mode 會改變更早的策略段；結束轉換的記錄通知會追加在可複用請求前綴之後。

### 結束工具 schema 與評審互動

#### 模型所見內容

[`exit_plan_mode` schema](../../../docs/tool-catalog.md#deepseek-aidsh-plan-mode) 在兩種狀態下均可用；在 plan mode 外執行會失敗，而 plan mode 內經批准的評審會返回規範的 `{ approved: true }` 值，並算繪既有的確認文字。拒絕仍是攜帶評審回饋的失敗呼叫，放棄審閱則是一次指明使用者接手的失敗呼叫。
...
- 如果行程在另一個被接受的輪內 pre-step 之前結束，某輪最後一個被接受的 pre-step 之後作出的選擇會丟失，因此 UI 必須重新應用它。
...
- 只有 Web UI 具備專用的 `plan-review` 算繪器；其他互動提供方可以透過通用選項流程呈現同一請求。
