...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-pipeline PipelineEditor id \'pipeline-editor\'',
    ],
    replaceRisk: 'none',
  ...
  {
    key: 'sidebar.pipelines',
    kind: 'single',
    scope: 'root',
    summary: 'Feature-navigation region below the session browser.',
    doc: 'Feature-navigation region below the session browser. Declared by this\npackage\'s \'sidebar\' entry; feature plugins (Pipelines today) register\ntheir navigation block here against the same section owner share.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-pipeline PipelinesNav',
    ],
    replaceRisk: 'shadows-shipped-ui',
