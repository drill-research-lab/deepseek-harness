...
- `approve(requestId)` / `decline(requestId)` / `startUserRun({ agentId, id, hasClientHalf })` — the two entries. All three are idempotent (per request id, and per definition for the user's own run), so a double press cannot start two runs. `hasClientHalf` is required on forwarded approvals and direct runs: a host-only definition has no source to fetch, so the Host event or panel caller supplies the Registry fact rather than the orchestrator inferring it from a failed fetch. An approved host-only request ends after `runHostHalf`; the Host settles its pending request.
