...
- `run` sends one approval round trip for both host-only and dual-half Packages. It emits `cordis/request-run` with `hasClientHalf`, suspends, and is settled by a person allowing or declining it. An answering page calls `runHostHalf`; for a host-only Package the Host commits and settles the request there, while a dual-half Package continues through Client loading and `resolveRequestRun`. There is no timer; the caller's `AbortSignal` is the only other way out, and it announces cancellation so every page removes its affordance. `run` has no wire face — `cordis_run` calls it in process.
