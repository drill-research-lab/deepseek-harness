...
export function apply(ctx: Context): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'even if the Plugin is currently stopped. An unauthorized Package creates an approval request and returns '
      + 'awaiting-approval; an authorized Package returns starting and continues asynchronously through an answering '
      + 'page. Host-only activation completes after its Host half starts; a dual-half Package then loads its Client '
      + 'half in that page. Neither result waits for the final outcome inside the Tool. currentPackageId changes only after '
