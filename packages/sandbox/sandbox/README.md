...
Policy rides the call, not the provider: two consumers may confine under different policies at the same instant (bash under `read-only` while a confined child agent keeps its state directory writable), and an approved escalated retry is just a new call with a wider policy. Filesystem consumers restrict reads to `workspaceRoot` in both confined modes; `read-only` additionally denies every mutation, while `workspace-write` permits mutations in the workspace and backend-defined temporary roots. Process backends may retain the system reads required to execute commands.

`SandboxExecutionPolicy.workspaceViewRoot` is the optional model-facing alias for `workspaceRoot`: a runner that rewrites every session workspace to one fixed path (the local Linux mount-namespace runners bind `workspaceRoot` to `/workspace`) sets it so the two views stay consistent. Enforcement always keys on the real `workspaceRoot`; `fromWorkspaceView` / `toWorkspaceView` (from [`roots.ts`](src/roots.ts), beside `readableRoots` / `writableRoots`) are the pure prefix maps an in-process filesystem Consumer applies to a model path in, and to a path it echoes back out. Identity when a deployment sets no view root.
