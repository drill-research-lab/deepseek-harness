...
The Linux runners bind the canonical host workspace root to `/workspace` and enter that path before executing the command. When `workspaceStorageRoot` contains the per-call workspace as a strict descendant, the runner then covers that storage root with an empty tmpfs, so its per-owner hash directories do not exist in the command's mount view while the preserved bind remains usable. Each bwrap process constructs both mounts itself; the Landlock rung asks [`@deepseek-ai/node-addon-pid-isolate-run`](../../../native/pid-isolate-run/) to create them after mount propagation becomes private and before setup capabilities are removed. The Landlock deployment must provide an existing `/workspace` directory as the bind destination, apply `setcap cap_sys_admin,cap_setpcap+ep` to the installed launcher, and verify `--probe`; otherwise the compound rung fails closed. Neither mount replaces the canonical host root or adds authority, and owner isolation remains rooted in the per-call source path.
