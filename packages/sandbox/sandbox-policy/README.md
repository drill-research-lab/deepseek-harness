...
The single owner of sandbox-policy resolution: the deployment's default and maximum [`SandboxMode`](../sandbox/README.md), permitted one-call escalation targets, and fallback root, plus each session's durable mode override and immutable workspace root. Every enforcing capability receives one resolved mode-and-root policy per call; before each request, the model receives the current policy without a separate capability inventory.
...
- `workspaceViewRoot` — an optional fixed path presented to the model when the configured runner maps every session workspace to one visible location. The shipped local Linux composition sets `/workspace`; other compositions leave it unset and present the canonical workspace root. When set, `resolve()` carries it on every returned `SandboxExecutionPolicy`, and in-process filesystem tools translate the prefix with `fromWorkspaceView` / `toWorkspaceView` ([`dsh-sandbox`](../sandbox/README.md)) so a `/workspace/...` path the model supplies reaches the real workspace root and every path echoed back reads as `/workspace/...`.

## API

- `ctx.sandboxPolicy.resolve({ session?, mode? })` — resolves one complete per-call policy. An explicit approved mode outranks the session's last `sandbox/mode` event, which outranks `defaultMode`; every source must stay at or below `maximumMode`. The session's immutable `cwd` is canonicalized with filesystem semantics before becoming `workspaceRoot`, otherwise the configured fallback applies. Canonicalization precedes lexical normalization so `symlink/..` agrees with process working-directory resolution. The resolved policy also carries `workspaceViewRoot` whenever the deployment configures one.
