...
- **`cordis-host-runner`** — disabled outright, belt-and-suspenders on top of the preset closure (its only model-facing surface, `tool-cordis`, already cannot appear in any approved preset).
- **`session-query-sqlite`** — `openAt: never` restated explicitly (already the value in the layers below); this bundle's startup check does not re-verify it directly, since no reachable override path exists.

## Startup policy check

`src/index.ts`'s `apply()` uses the same pure validator as its unit tests. After every patch layer has applied, it requires the exact approved preset set `{drill-production}`, the exact default `drill-production`, `includeUserRoot: false`, and the exact permission table `{read-only, workspace-write}` with their expected sandbox and approval values. It throws instead of silently clamping, so later deployment patches cannot widen the policy unnoticed.
...
- **The startup check does not independently re-verify `session-persistence-sqlite` or `session-query-sqlite`'s `openAt`.** The production patch keeps the query index at `openAt: never`; it is not a host-execution path and remains a product data-indexing choice rather than an A3 startup dependency.
