...
- 互動式前臺子行程（例如 REPL）只有在行程管理提供方能證明其 stdin 等待時才會提前返回部分輸出；否則呼叫會一直執行到 `timeoutMs`。
