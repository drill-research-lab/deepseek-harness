...
`writeFileAtomic` 提交一份已經算繪好的字串。約定按故障利用它的先後順序列出：
...
`withFileLock` 跨行程序列化同一文件的寫入方，服務於單靠原子提交無法保證安全的讀-算繪-提交迴圈。鎖是以 `wx` 建立的同目錄 `<filename>.lock`，因此讀取方從不參與競爭；等待方按指數退避，逾時即失敗而非無限阻塞。競爭者絕不移除現有鎖：鎖齡無法區分已經崩潰的所有者與被暫停但仍存活的寫入方。
...
- **殘留鎖需要操作者復原**——行程持鎖結束時可能留下同級鎖定檔。後續寫入方逾時也不會刪除它；操作者只有在確認沒有寫入方仍擁有該鎖後才會移除。文件存續時間本身不能安全證明它已無人持有。
