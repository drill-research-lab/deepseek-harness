/**
 * Path-containment mechanics shared by every consumer that must keep a
 * model- or user-chosen path inside a trusted root. Canonical spellings take
 * the fast lexical path; filesystem identity supplies the conservative
 * fallback for alias-equivalent roots such as Windows 8.3 names and casing.
 * @module @deepseek-ai/dsh-path-containment
 */
 ...
 * walk the target's existing ancestors and compare filesystem identity with
 * the root; this recognizes Windows long-name/8.3 aliases and casing without
