...
提供方信任其配置的服务器，不限制该服务器进程。工作区元数据与源文件读取会携带一个以规范化工作区为根的 `read-only` 按调用文件系统策略，因此它们仍可通过 `fs-sandbox` 读取，同时模型提供的源文件仍限制在该工作区内。它把规范化身份、包含关系、普通文件流式读取、UTF-8 验证和文件 URI 编码委托给 `ctx.fs`；并在服务器启动前拒绝缺失、非普通文件、非 UTF-8、过大，或规范化后位于工作区外部的查询源。包含关系在打开流之前评估，不承诺在并发路径替换期间保持稳定句柄身份。结果位置可以在外部，但外部路径不能成为查询源。部署必须挂载描述同一执行世界的文件系统与进程管理提供方；分裂世界组合无效。
...
- **不提供进程隔离策略**：源文件读取受工作区限制，但本包（package）信任所配置的服务器，不对其进程实施沙箱；受限部署必须提供适当的进程提供方，或使用同一执行世界的沙箱包装层。
