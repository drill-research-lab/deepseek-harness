...
每个请求都携带 dsh-llm `attributionHeaders()` 的共享归因标头，并在 `GenerateOptions.sessionId` 存在时携带 Harness 会话标头 `x-deepseek-harness-session-id`；两者都通过 pi-ai `headers` 流选项合并，且都优先于冲突的已配置标头名。不会合成提供方特定应用归因标头。详见 [dsh-llm § 应用归因](../llm/README.md#app-attribution-attributionts)。
