...
worker 錯誤、訊息失敗或提前結束會在清理前關閉訊息接納，然後以 `error` 兌現；如果取消已經接管該執行，則不覆蓋取消。後到的排隊訊息無法在該邏輯邊界後建立子 agent 或寄出敘述。

宿主會維護已轉發子 agent 啟動的臺帳。優雅結束的 worker 會提供對應的結束事件；死亡或強制終止會把缺失的結束事件合成為已取消。因此，每個已轉發的 `workflow/agent-start` 都會且只會配對一次，不過已經到達的工作流程結果之後的清理可能稍後才完成。
