...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  'packages/identity/auth': { kind: 'none', reason: 'Identity contracts and request-local state register no prompt, schema, tool, or provider request.' },
  'packages/identity/auth-ldap': { kind: 'none', reason: 'The LDAP adapter runs only in the external authentication gateway and registers nothing model-facing.' },
  ...
  'packages/client/ui-inference-dashboard': { kind: 'none', reason: 'Browser-side operational metrics surface; registers nothing model-facing.' },
  'packages/client/ui-admin-queue': { kind: 'none', reason: 'Browser-side admin queue surface; reads and reorders admission-queue metadata and registers nothing model-facing.' },
  ...
  'packages/llm/llm-admission-queue': { kind: 'none', reason: 'The admission gate defers only the send timing of a gated request and changes no prompt, schema, tool, response, or session event.' },
