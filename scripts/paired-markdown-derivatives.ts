 ...
 * checks performed on their unsuffixed English siblings. The language-pairing
 * gate owns cross-language identity; source-oriented gates consume one copy.
 */

/** The result of separating canonical blocks from paired translated derivatives. */
...
/** Return the unsuffixed English sibling of any translated Markdown path. */
function unsuffixedSibling(doc: string): string | null {
  return doc.endsWith('.zh.md')
    ? `${doc.slice(0, -'.zh.md'.length)}.md`
    : doc.endsWith('.zh-tw.md')
      ? `${doc.slice(0, -'.zh-tw.md'.length)}.md`
      : null
}
 ...
 * @returns Primary blocks and paired translated derivatives, preserving order.
 */
