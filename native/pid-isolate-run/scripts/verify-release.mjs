 ...
 * `pid-isolate-run-vX.Y.Z` tag matches it. With `--prebuilds`: every platform package's declared
...
const TAG_PREFIX = 'refs/tags/pid-isolate-run-v';
...
function verifyVersions() {
  ...
  if (publish && !ref.startsWith(TAG_PREFIX)) {
    throw new Error('publishing requires running the workflow from a pid-isolate-run-v* tag');
  }
  if (ref.startsWith(TAG_PREFIX)) {
    const tagVersion = ref.slice(TAG_PREFIX.length);
    if (tagVersion !== version) {
      throw new Error(`tag/version mismatch: tag pid-isolate-run-v${tagVersion}, packages ${version}`);
    }
  }
