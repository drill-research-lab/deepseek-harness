 ...
 * because a partial publication can leave no way forward: republishing an
...
async function publishTarball(tarball, name, version) {
  ...
  for (let tries = 1; tries <= PUBLISH_ATTEMPTS; tries += 1) {
    ...
    if (settled.kind === 'present' && settled.integrity === integrityOf(tarball)) {
      console.log(`pid-isolate publish: ${name}@${version} landed despite a reported failure, continuing`);
      return;
    }
    ...
    console.log(
      `pid-isolate publish: ${name}@${version} hit a transient registry failure`
...
for (const filename of order) {
  ...
  if (state.kind === 'present') {
    ...
    console.log(`pid-isolate publish: ${name}@${version} already published, skipping`);
    skipped += 1;
    continue;
  }
  ...
  console.log(`pid-isolate publish: ${name}@${version} published`);
  published += 1;
}

console.log(`pid-isolate publish: ${published} published, ${skipped} already present`);
