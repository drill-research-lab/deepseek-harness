...
The release binary requires exactly `cap_sys_admin,cap_setpcap+ep` at deployment. Preserve the setup order, the parent/child capability verification, exit code `125`, fatal prefix, and exact probe output. `DROP_NOOP` and `CAPBSET_READ_FAIL` are test-only and must never enter a platform package.
