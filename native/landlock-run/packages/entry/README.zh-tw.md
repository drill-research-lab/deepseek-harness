...
啟動器在自身上安裝 Landlock 規則集，再 `exec` 被包裝的命令；該規則集會跨 `execve` 繼承，因此整個行程樹都在限制下執行。未授予的一切都被拒絕；啟動器失敗時以 `125` 結束且不執行命令：採用失敗閉合策略，絕不在失敗時放行。二進位約定鎖定在倉庫的 `docs/cli-contract.md` 中；C 原始碼作為 `src/main.c` 隨該 tarball 分發，便於審計。
