...
An implemented Agent Note that is fully superseded may be consolidated into the current owning note and deleted. Before deletion, the owner must preserve every unique rationale, alternative, consequence, required verification, and named coverage gap; repair every inbound link; and delete the consistency record in the same change. Partial supersession does not qualify: keep both notes cross-linked and update every fact that remains current. Consolidation must not rewrite the old file into its opposite or rely on git history as the only copy of rationale.
...
This fork keeps Agent Notes English-only; no `.zh.md` counterpart is written for active notes. The frozen `archived/` tree retains the bilingual triplets inherited from upstream exactly as sealed — never add, edit, or delete a `.zh.md` there.
