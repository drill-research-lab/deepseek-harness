...
手工驅動程式注意：`bin` 將 stdin EOF 視為「用戶端已離開」並立即 dispose，生命週期較短的管道會中止進行中的輪次——管道驅動程式必須保持 stdin 打開，直到輪次結束。
