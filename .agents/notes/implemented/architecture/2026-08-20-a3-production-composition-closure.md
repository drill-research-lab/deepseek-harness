...
Package tests exercise roster filtering, exact startup-policy rejection (including a mounted `dynamicCordisRunner` and a non-`'never'` `session-query-sqlite` `openAt`), real mounting, and JSONL-backed restart denial. The real production boot spawns `base`+`web-app`+`drill-production`, mounts the resolved preset, inspects its runtime tool schemas, confirms both permission ids and the absent dynamic Cordis runner, rejects `cordis`/`minimal`/`standard` over HTTP, rejects a Full Access settings mutation, proves all directory-picker RPC methods return `directory-picker-unavailable`, and spawns two further real boots — one with a `cordis-host-runner: disabled: false` overlay patch, one with a `session-query-sqlite` `openAt: 'startup'` overlay patch — confirming each now fails boot with the corresponding named diagnostic instead of reaching ready.
