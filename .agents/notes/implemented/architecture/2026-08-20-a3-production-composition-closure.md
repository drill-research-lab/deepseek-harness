...
**Directory picker: closed, not confined, because no safe root exists.** `directory-picker-browse`/`workspace.create` both accept arbitrary absolute paths with no containment (re-confirmed against current source, not assumed from the earlier audit). Confining the picker to `UserHome` is explicitly wrong — that directory holds sessions, credentials, and other DSH-private data, not a user-facing workspace root. `packages/host/apiproxy` hard-requires `ctx.directoryPicker` (`inject: [...]`), so the row cannot simply be `disabled: true` with nothing replacing it; `packages/bundle/drill-production/src/startup.ts`'s `DisabledDirectoryPicker` declaration-merges a new `disabled` capability kind into `DirectoryPickerCapabilities` (the package's own documented extension point) and reports it — the client's documented default for an unrecognized kind is to hide the picking affordance, not fail.

**`cordis-host-runner`: disabled outright, belt-and-suspenders.** Its only model-facing surface (`tool-cordis`) already cannot appear in any approved preset, but the host-plane service is additionally disabled so "cordis-host-runner absent" holds unconditionally rather than "unreachable through every preset this deployment currently ships."

**`session-persistence-sqlite`/`session-query-sqlite`: restated, not re-fixed.** Both were already closed before this change (the former never mounted at all, the latter dormant under `openAt: never` with no reachable override — confirmed via the same exhaustive trace as the Phase 0 audit, not re-derived). This bundle restates `session-query-sqlite`'s `openAt: never` explicitly as a self-documenting invariant; it does not add code, per the prior Agent Note's own instruction that A3 "must not re-investigate this from scratch."

**A startup FAIL LOUD assertion, not silent trust in the config.** `validateDrillProductionPolicy()` is shared by startup and unit tests. After every patch layer it requires the exact approved set `{drill-production}`, default `drill-production`, `includeUserRoot: false`, and the exact permission ids and mappings for `read-only` and `workspace-write`. It throws a named diagnostic rather than clamping.
...
| Directory picker (native/browse) | `directory-picker-auto`/`-browse` | No safe, non-`UserHome` browse root exists in the current architecture | A real user-facing workspace/data root (Issue #7 / Product Workspace territory), or a genuinely confined browse boundary | Re-enable once a safe root exists, confined to it — not to whole-filesystem scope |
...
Package tests exercise roster filtering, exact startup-policy rejection, real mounting, and JSONL-backed restart denial. The real production boot spawns `base`+`web-app`+`drill-production`, mounts the resolved preset, inspects its runtime tool schemas, confirms both permission ids and the absent dynamic Cordis runner, rejects `cordis`/`minimal`/`standard` over HTTP, rejects a Full Access settings mutation, and proves all directory-picker RPC methods return `directory-picker-unavailable`.
...
Drill multi-user production, with this bundle composed, has no Bash, PowerShell, or generic-filesystem model tool, no dynamic self-modification, no arbitrary directory browsing, and no `danger-full-access` selection surface — by design, not by omission, and re-openable only through the Issue #5 handoff matrix above. A generic `dsh-web-app`/single-user deployment is unaffected. The startup check turns a future accidental widening (an operator's patch layer re-adding `danger-full-access` to the table, or clearing `approvedIds`) into a boot failure with a named diagnostic rather than a silent regression.
