...
`packages/fs/fs-sandbox/` (`@deepseek-ai/dsh-fs-sandbox`) mirrors the `bash-local`/`bash-sandbox` split: `SandboxedFileSystem extends LocalFileSystem`, registered as `ctx.fs`, injecting `sandboxPolicy`. The six read operations (`stat`/`lstat`/`readText`/`streamText`/`readBytes`/`listDir`) and the two mutations receive the same per-call policy before delegating to the inherited local operation:

- Under either confined mode, reads fence the canonical target against `readableRoots(policy)`, which contains only the calling session's workspace root. `lstat` canonicalizes the parent but deliberately does not follow the final component, so it can inspect an in-workspace symlink entry without granting access to its external target. The other reads receive an already resolved target and deny a final symlink that resolves outside.
...
A denial is the structured `FS_SANDBOX_DENIED` carrying the effective mode — distinct from `FS_PERMISSION_DENIED` (a host EACCES is the world refusing; this is policy refusing). No text inference: an in-process fence knows exactly what it denied. The per-call carrier is a trailing optional `SandboxExecutionPolicy` on all six reads and both mutations (the filesystem twin of `ShellExecRequest.sandboxPolicy`); the seam stays session-free, and bare local and E2B backends ignore it. `FileSystem.sandboxMode` is the capability fact (`undefined` on the base and bare backends, the default on `SandboxedFileSystem`), so the tool layer resolves policy only when composition requires it and advertises mutation escalation from composition truth.

Trusted internal loaders whose configured inputs intentionally live outside a model session workspace use `readPolicyForTarget(fs, target)`. It creates a `read-only` per-call policy rooted at that already-resolved target's execution-world process path: `skill-filesystem` roots configured skill and project-marker reads at their home or project directory; `agent-instructions` roots configured instruction and project-marker reads the same way; `lsp-stdio` roots workspace metadata and source reads at the canonical workspace. This is a least-privilege read grant, not `danger-full-access`: the configured root is readable, mutations remain denied, and symlink escapes or sibling paths remain fenced. Model-facing fs tools and search continue to carry the calling session's resolved policy.

`dsh-tool-fs-search` remains process-backed rather than joining the `ctx.fs` seam. Each `glob` or `grep` call resolves the same session policy, rejects an explicit search root outside the canonical workspace (including a final symlink escape), prepends `--no-config`, and spawns only the argv returned by `ctx.sandbox.confine()` for confined modes. This userspace root check closes the model-directed read surface without claiming the general shell-code read isolation owned by the process sandbox.
...
- Under `read-only`, `write`/`edit` return the `[sandbox: file access denied under read-only mode]` marker and the disk is untouched; reads succeed only within the calling session workspace.
- Under `workspace-write`, Alice's six fs read operations cannot inspect Bob's workspace or another absolute host path; each succeeds within Alice's workspace, and `lstat` preserves no-follow final-symlink semantics.
- `glob`/`grep` cannot accept an external canonical search root, retain `--no-config`, and route every confined run through `ctx.sandbox.confine()` before `ctx.subprocess.spawn()`.
...
- Trusted skill and instruction loaders can read their configured home/project targets through `dsh-fs-sandbox`, and LSP source reads can use a workspace outside the deployment fallback; ordinary unstamped reads and model-supplied LSP paths outside that workspace remain denied.
...
- Unit: `dsh-sandbox` pins the escalation ladder, marker builders, argument-pairing validation, `approveEscalation`, and `readableRoots`/`writableRoots`/`canonicalPath`. `dsh-sandbox-policy` pins deployment fallback, session mode/root resolution, explicit-mode precedence, the fold/setter, load-time mode rejection, and HMR safety. `dsh-fs-sandbox` pins all six reads inside the workspace, outside it, and across owners, plus final-symlink `lstat` semantics and the mutation containment matrix. Composition tests pin trusted skill, instruction, and LSP reads through `dsh-fs-sandbox` while proving unstamped or out-of-workspace model paths remain denied. `dsh-tool-fs` pins read-policy propagation, advertisement gating, denial-marker mapping, and mutation escalation. `dsh-tool-fs-search` pins canonical root rejection, `--no-config`, sandbox wrapping, JSON parsing, spill recovery, and the `SEARCH_*` vocabulary. `dsh-tool-bash`, `dsh-bash-sandbox`, and `dsh-permission-presets` consume the same policy kit.
