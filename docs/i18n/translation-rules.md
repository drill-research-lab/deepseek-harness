...
- Each counterpart *MUST* say what the authored side says — no added behavior, prerequisites, warnings, version claims, or examples, and no dropped ones. If the pair disagrees on substance, no language wins by default: fix the side that is wrong, then bring the other counterparts along in the same change.
- Each counterpart *SHOULD* read as natural technical writing in its own language, not word-by-word gloss. Translate meaning, restructure sentences where the target grammar wants it, and keep the author's register — terse stays terse.
...
- links and anchors: every relative link MUST point at the same target in all files — by convention the `.md` path, not a translated sibling — so links never dangle when one pair lands before its neighbors. The only switcher links are the language links. A README rendered outside GitHub MAY use the canonical public repository URL to its exact counterparts as documented in [README.md](README.md). Link TEXT is translated; the target is not.

The repo's Markdown conventions apply to `.zh.md` and `.zh-tw.md` files unchanged: one physical line per paragraph (`verify-md-wrap`), resolving relative links (`verify-md-links`), exactly one trailing newline.
...
- zh-TW renderings follow [terminology-zh-tw.md](terminology-zh-tw.md), the zh→zh-TW conversion glossary, for Chinese renderings; the terminology table's English anchor governs cross-checks.
- The zh-TW track is mechanically converted from zh-CN, then reviewed against English. Use 教育部（Taiwan MoE）typography conventions, choose 您 or 你 according to Taiwan technical-writing practice, and apply [terminology-zh-tw.md](terminology-zh-tw.md) for renderings. The Simplified-specific “你 not 您” rule does not automatically apply to zh-TW.
...
The existing typography rules govern the Simplified Chinese side; zh-TW follows Taiwan conventions including 教育部《重訂標點符號手冊》. Both Chinese sides share the mixed-script spacing rules below.
