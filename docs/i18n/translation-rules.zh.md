...
[English](translation-rules.md) | 简体中文

本文规定：如何在本仓库文档配对的英文、简体中文与繁体中文三种语言之间进行翻译。所有语言同权（见 [README.md](README.md)）：每次变更可以用任一语言撰写，被编辑的一侧即为本次更新的源；本文的规则约束如何产出或更新其他对侧文件。这些规则对人类和 agent（智能体）同等生效。日常工作中，agent 会在术语指导下直接一次完成有改动内容的翻译；扩展版 [.agents/skills/dsh-translate-docs](../../.agents/skills/dsh-translate-docs/SKILL.md) 工作流仅在用户显式调用时运行。规则级别沿用 RFC 2119 的用法：**必须（MUST）** ／ **禁止（MUST NOT）** 会卡门禁或评审；**应当（SHOULD）** 偏离时要说明理由；**可以（MAY）** 自行裁量。

## 忠实性

- 每个对侧文件*必须*传达与撰写侧相同的内容：不添加行为、前置条件、警告、版本声明或示例，也不漏掉任何一项。如果配对文件在实质内容上不一致，没有哪种语言默认获胜；请修正错误的一侧，并在同一个变更里同步更新其他对侧文件。
- 每个对侧文件读起来*应当*是其语言自然的技术文字，而非逐词对照的译文。请根据语义翻译，在目标语言语法需要时重组句子，并保持原作者的语域（比如：简练的保持简练）。
- 不要翻译不可译的内容：如果一句话依赖源语言的习语、无法自然转换，请翻译它的意思，而非习语本身。

## 行文

- 语体以 [style-samples.md](style-samples.md) 为校准锚点。人工定稿的黄金样例按文体各一组，译文必须参照文体最接近的样例，采用其中目标语言一侧的语体；如果样例与本文的行文规则冲突，以样例为准。译成中文时，采用规范的技术制度文；译成英文时，采用简洁、专业的开发者文档语体。
...
- 链接与锚点：每个相对链接在所有文件中必须指向相同的目标（按约定是 `.md` 路径而非翻译侧文件），这样即使某对文档先于相邻文件落地，链接也不会悬空。只有语言切换行使用翻译侧链接。在 GitHub 以外位置渲染的 README 可以按 [README.md](README.md) 的规定，使用指向确切对侧文件的规范公开仓库 URL。链接**文字**翻译；链接目标不翻。

本仓库的 Markdown 约定对 `.zh.md` 与 `.zh-tw.md` 文件原样生效：一个段落一个物理行（`verify-md-wrap`）、相对链接必须可解析（`verify-md-links`）、文件末尾恰好一个换行。

## 术语

- [terminology.md](terminology.md) 是双向的术语真源。翻译前请先加载它；表内术语必须遵守对应行与「不要译作」禁项。译成中文时，采用「中文」列，并按「首次出现」列括注；译成英文时，采用「English」列，不加中文括注。
- 繁体中文的术语写法遵循 [terminology-zh-tw.md](terminology-zh-tw.md)，该文件是 zh→zh-TW 的转换术语表；术语表的 English 锚点负责交叉核对。
- zh-TW 路径先由 zh-CN 机械转换，再对照英文评审。排版采用教育部《重訂標點符號手冊》等台湾规范；「您」或「你」按台湾技术写作惯例选择，并使用 [terminology-zh-tw.md](terminology-zh-tw.md) 规定的写法。简体中文专用的「你而非您」规则不自动适用于 zh-TW。
...
现有排版规则约束简体中文一侧；zh-TW 遵循包括教育部《重訂標點符號手冊》在内的台湾规范。以下混合文字间距规则由两种中文共同遵守。
