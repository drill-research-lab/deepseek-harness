<!-- 英文原始檔由 scripts/gen-module-graph.ts 生成；本中文文件是透過雙語配對維護的經評審對側。
     更新時先執行 `pnpm run gen-module-graph` 更新英文，再更新本文件並執行 `pnpm run verify-translation-pairing --write docs/module-graph.md` 重新記錄配對。 -->
