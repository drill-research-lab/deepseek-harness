...
`GoalId` 是[品牌化 id](core.md#branded-ids)。呼叫方透過 `GoalRef` 修改一個確切修訂版本；每次獲準的持久變更都會遞增修訂號。
...
持久階段回答目標發生了什麼。行程本機啟用狀態則另行回答續跑消費端能否開始另一個 Round。
...
阻塞是唯一表示「因問題而停止」的持久狀態。由策略負責的阻塞原因會攜帶一個用於路由、穩定且採用 lower-kebab-case 的程式碼，以及一段供人和模型閱讀的自由文字說明。
...
每次變更都是持久的 `goal/change` 工作階段事件，其載荷要麼是變更後的完整快照，要麼是清除墓碑。嚴格摺疊與持久投影只從這些事件派生生命週期狀態；inbox 變更不會影響 goal 狀態。
...
續跑消費端會為每個獲準的使用者訊息輪次標注正數且連續的 Round 編號和當前修訂號；只有這些獲準的 `user/message` 事件會推進 `roundsStarted`。重播會拒絕非正數 Round、編號缺口、過時修訂號、已停止階段和超出上限。
...
建立操作會區分呼叫方省略欄位與採用部署設定值這兩種情況，`create()` 會在內部解析後者。編輯是區域性替換，其執行時期校驗器要求至少提供一個欄位。每條變更通知都會攜帶獲準的操作和確切修訂號；清除操作不帶 `goal`。
...
[`GoalService`](../../packages/goal/goal/src/index.ts) 解析建立預設值、從持久 `goal/change` 事件執行嚴格重播摺疊、校驗傳入的 agent（代理）是登錄檔中的確切活躍實例、以比較並設定方式執行變更，並行出 `goal/changed` 通知；監聽器故障會被隔離。包 [README](../../packages/goal/goal/README.md) 定義可呼叫 API 和麵向模型的約定。
