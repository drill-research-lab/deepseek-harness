...
Service Definition: [`dsh-pipeline`](../../packages/pipeline/pipeline) (`ctx.pipelineEngine` + the vocabulary below). The engine provider — the file-backed registry, the cron scheduler, and the DAG evaluator over run sessions — and the model-facing and UI consumers land in later slices of the same effort. `save` is the durable parser boundary: every persisted definition, whatever handed it in, passes `validateWorkflowJson` first. `startRun` applies the overlap policy (one executing run per pipeline; further triggers are skipped and reported as data, never queued), and every mutation emits `pipeline/definition-changed` only after the registry commits. Run data itself lives in each run's own session log; the `pipeline/*` events deliberately carry no node input or output values, so a listener observing lifecycle can never gain a mutable alias of run data.
