...
日誌是按寄出順序排列的純字串。執行時期捕獲程序的 console 與流輸出，但通道和 console 方法的中繼資料不屬於 seam，因為 Consumer 只算繪文字。實作會對序列化後的外層日誌陣列，以及完成值或失敗訊息的組合載荷設定上限；固定的結果封裝文法與 Consumer 展示空白不計入這份可變載荷計量。超限會顯式失敗，而不會在值中插入替代內容。
